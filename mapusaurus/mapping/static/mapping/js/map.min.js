...
function initCalls(geoQueryType) {
    ...
    "selected" === gt ? (blockStuff(), $.when(getTractData(action, !1, !1)).done(function(data1) {
        init(data1);
    })) : "all_msa" === gt ? (blockStuff(), $.when(getTractData(action, getBoundParams(), "msa")).done(function(data1) {
        init(data1);
    })) : "all" === gt && (blockStuff(), $.when(getTractData(action, getBoundParams(), !1)).done(function(data1) {
        init(data1);
    }));
}
...
var rawGeo, rawLar, rawMinority, rawData, selectedYear = selectedYear || 2014, isUIBlocked = !1, larVolume = [], pctMinority = [], lastRequest, dataStore = {};

dataStore.tracts = {}, $.ajaxSetup({
    beforeSend: function(jqXHR) {
        lastRequest && lastRequest.abort(), lastRequest = jqXHR;
    }
});
